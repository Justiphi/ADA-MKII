...
**This decision still stands after reminders — the exception the plan expected was not needed.** Android reminders were assumed to require a local copy of upcoming events so they could fire offline. They do not: `ReminderSync` hands occurrences to the platform's own scheduler, which owns them from that moment and delivers them with ADA closed and the phone offline. ADA stores nothing, so there is no stale copy, no write path, no outbox and no conflict rules — the actual reasons a cache was ruled out. What it costs instead is that reminders only exist as far ahead as the last successful sync reached (30 days); leave the app unopened longer than that and they run out. **They also do not survive a reboot** — `Plugin.LocalNotification` ships no `BOOT_COMPLETED` receiver and Android clears `AlarmManager` entries on restart, so reminders resume when the app is next opened. Fixing that properly *would* require the stored copy this design avoids. Server-side push removes both limits and remains the upgrade path.
...
6. **Android head — COMPLETE and run.** Manifest permissions and `ApplicationId` landed earlier; this phase added the runtime-configurable server address (`IServerAddressProvider` plus a field on the login screen) and an Android network security config that keeps cleartext HTTP off everywhere except the named development hosts. **Verified on a Pixel 8 Pro emulator, Android 16 / API 36**: sign-in, session restore from `SecureStorage`, streaming chat, the tool loop, notes, calendar, memories and weather all work against a server on `10.0.2.2:5100`. Running it found three things no build could: the Fast Deployment crash (see the `ada-dev` skill), the system-bar and keyboard insets (item 6a), and a reminder sync that never ran (item 10).

    **6a. System bars and the keyboard — fixed after the first run.** `targetSdkVersion=36` means Android lays the activity out edge-to-edge and, from Android 16, ignores the opt-out. The `BlazorWebView` therefore drew *under* the status bar, which put the shell header inside the bar's touch region: `Settings` and `Sign out` were not merely ugly, they were **impossible to press**. The keyboard was the mirror image — no `windowSoftInputMode`, so the WebView never resized and the chat composer sat behind the IME.

    Both are handled natively in `MainActivity`: `WindowSoftInputMode = SoftInput.AdjustResize`, plus an `OnApplyWindowInsetsListener` that pads the content view for `SystemBars() | Ime()`. **Not** CSS `env(safe-area-inset-*)` — in Android WebView those resolve against the display cutout rather than the system bars, so on a device without a notch they are simply `0` and the header stays under the clock. The padding exposes the window background behind the now-transparent bars, so `ada_window_background` matches `--ada-bg` and the bar icons are forced light.
...
10. **Android reminders — Stage 3 COMPLETE, and delivery verified.** `IReminderScheduler` with `LocalNotificationReminderScheduler` on MAUI and `NullReminderScheduler` elsewhere; `ReminderSync` in `UI-Shared` pushes 30 days of reminder-enabled occurrences to the device on app start, throttled to six hours. `POST_NOTIFICATIONS` is declared and prompted for at runtime; `SCHEDULE_EXACT_ALARM` deliberately is not, so the schedule mode is pinned to `InexactAllowWhileIdle` — the plugin's default is `setExactAndAllowWhileIdle`, which would need that restricted permission and fail on Android 12+. Verified by reading the merged `obj/.../android/AndroidManifest.xml`: the permission is present, the exact-alarm one is absent, and the plugin's `ScheduledAlarmReceiver` is registered.

    **A reminder has now been delivered on a device** — runtime prompt granted, alarm registered against `ScheduledAlarmReceiver` at exactly `start - 15min`, and the notification posted on the `ada_reminders` channel with the app backgrounded.
...
- **Weather needs no API key, and that was the deciding factor.** Open-Meteo is free and keyless, so a fresh deployment reports weather before any credential exists — the same property that makes `EchoLlmProvider` worth having. It is still called server-side: a head that reaches a third-party service directly is the pattern this topology exists to prevent, and making an exception for the one provider that happens to need no key is how that rule quietly stops being true.
- **A place name needs its country.** Searching Open-Meteo for "Tauranga" returns a town in **Sudan** as its second hit. Anything after the comma in `weather.location` filters by country code or name, which is why the default is `Tauranga, NZ` and not `Tauranga`.
