...
/// <summary>
/// Token accounting for the spend guard, scoped to the current account so one
/// user cannot exhaust another's budget. Sums the per-message counts recorded by
...
/// </summary>
public sealed class SqlUsageStore(AdaDbContext db, IAccountContext account) : IUsageStore
{
    public async Task<long> GetTokensSinceAsync(DateTimeOffset since, CancellationToken cancellationToken)
    {
        ...
        return await db.Messages
            .AsNoTracking()
            .Where(m => m.CreatedUtc >= since && m.Conversation!.AccountId == account.AccountId)
